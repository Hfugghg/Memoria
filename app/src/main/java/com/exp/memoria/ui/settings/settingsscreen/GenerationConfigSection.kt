 ...
 * 并通过挂起函数将用户的修改通知给 `SettingsViewModel`。
 *
 * ### 主要功能:
 ...
 * @param onTemperatureChange 当 Temperature 值变化时调用的挂起函数。
 * @param onTopPChange 当 Top-P 值变化时调用的挂起函数。
 * @param onTopKChange 当 Top-K 值变化时调用的挂起函数。
 * @param onMaxOutputTokensChange 当最大输出 Token 数变化时调用的挂起函数。
 * @param onStopSequencesChange 当停止序列变化时调用的挂起函数。
 * @param onFrequencyPenaltyChange 当频率惩罚值变化时调用的挂起函数。
 * @param onPresencePenaltyChange 当存在惩罚值变化时调用的挂起函数。
 * @param onCandidateCountChange 当候选数量变化时调用的挂起函数。
 * @param onSeedChange 当随机种子变化时调用的挂起函数。
 * @param onResponseMimeTypeChange 当 Response MIME Type 变化时调用的挂起函数。
 * @param onResponseLogprobsChange 当是否返回对数概率的开关变化时调用的挂起函数。
 * @param onOutputDimensionalityChange 当嵌入输出维度变化时调用的挂起函数。
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun GenerationConfigSection(
    settings: Settings,
    onTemperatureChange: suspend (Float) -> Unit,
    onTopPChange: suspend (Float) -> Unit,
    onTopKChange: suspend (Int?) -> Unit,
    onMaxOutputTokensChange: suspend (Int?) -> Unit,
    onStopSequencesChange: suspend (String) -> Unit,
    onFrequencyPenaltyChange: suspend (Float) -> Unit,
    onPresencePenaltyChange: suspend (Float) -> Unit,
    onCandidateCountChange: suspend (Int) -> Unit,
    onSeedChange: suspend (Int?) -> Unit,
    onResponseMimeTypeChange: suspend (String) -> Unit,
    onResponseLogprobsChange: suspend (Boolean) -> Unit,
    onOutputDimensionalityChange: suspend (Int?) -> Unit
) {
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            OutlinedTextField(
                ...
                onValueChange = { scope.launch { onStopSequencesChange(it) } },
                label = { Text("停止序列") },
            ...
            OutlinedTextField(
                ...
                onValueChange = { scope.launch { onResponseMimeTypeChange(it) } },
                ...
                ExposedDropdownMenu(
                    ...
                    outputDimensionalityOptions.forEach { (value, label) ->
                        DropdownMenuItem(
                            text = { Text(label) },
                            onClick = {
                                scope.launch { onOutputDimensionalityChange(value.toIntOrNull()) }
            ...
            Row(
                ...
                Switch(
                    ...
                    onCheckedChange = { scope.launch { onResponseLogprobsChange(it) } }
                )
            }
        }
    }
